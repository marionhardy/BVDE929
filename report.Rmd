...
# Unsatisfying, there's a problem i think

lall %>% 
  drop_na(Abundance) %>% 
  group_by(Treatment,state) %>% 
  summarise(mean = mean(`Abundance Ratio: (F2, H2O2) / (F2, Non R)`))

lall %>% 
  filter(state == "Oxidated"&Treatment == "F2_H2O2"|
         state == "Not oxidated"&Treatment == "F2_ctrl") %>% 
  drop_na(Abundance) %>% 
  group_by(Treatment,state) %>% 
  summarise(mean = mean(`Abundance Ratio: (F2, H2O2) / (F2, Non R)`))
