...
all = left_join(data, data1, by = c("Annotated Sequence" = "Sequence", "state" = "state", "Treatment" = "Treatment"))
...
Il y a 49 peptides qui sont oxidés en H2O2 et non oxidés en ctrl.

```{r}

length(unique(targetpep$`Annotated Sequence`))

```


### All oxidated peptides with a match in the other condition

```{r}

ox = lall %>% filter(state == "Oxidated") %>% 
  select(`Annotated Sequence`, Modifications.x, Abundance, state, Treatment, Retention_time) %>% 
  drop_na(Abundance)

unox = lall %>% filter(state == "Not oxidated") %>% 
  select(`Annotated Sequence`, Modifications.x, Abundance, state, Treatment, Retention_time) %>% 
  drop_na(Abundance)
...
targetpepall = inner_join(ox, unox, by = c("Annotated Sequence","Treatment"), suffix = c("_oxidated", "_unoxidated"))


lall %>% 
  filter(Treatment %in% c("F2_H2O2","F2_ctrl")) %>% 
  filter(`Annotated Sequence`%in% targetpepall$`Annotated Sequence`) %>% 
  head(150) %>% 
  ggplot(aes(y = `Annotated Sequence`,
             x = Retention_time,
             color = state, 
             fill = state,
             shape = Treatment))+
...
Il y a 48 peptides qui sont oxidés et non oxidés dans la condition cxtrl

```{r}

length(unique(targetpepall$`Annotated Sequence`))
